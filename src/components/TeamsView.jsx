...
const TeamsView = () => {
  const { teams, updateTeam, addPlayer, removePlayer, settings, updatePlayer } = useTournament();
  ...
  const handleAddPlayer = () => {
    if (newPlayerInput.trim() && selectedTeam.players.length < settings.playersPerTeam) {
      addPlayer(selectedTeamId, newPlayerInput.trim());
      setNewPlayerInput('');
    }
  };
          ...
          >
            ...
            <div className="team-card-player-list mt-2">
              ...
              {team.players.slice(0, 6).map(p => (
                <div key={p.id} className="player-name-chip" title={p.name}>
                  {p.name}
                </div>
              ))}
              {team.players.length > 6 && (
                <div className="player-name-chip more">+{team.players.length - 6} more</div>
              )}
            </div>
          </div>
        ))}
